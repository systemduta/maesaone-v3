...
<div class='form-group {{!isset($header_group_class)}} row' id='form-group-{{$name}}'>
    @if($form['columns'])
        <div class="col-sm-12">
                    ...
                    @push('bottom')
                        ...
                        <script type="text/javascript">
                            ...
                            function deleteRow{{$name}}(t) {
                                if (confirm("{{trans('locale.delete_title_confirm')}}")) {
                                    $(t).parent().parent().remove();
                                    if ($('#table-{{$name}} tbody tr').length == 0) {
                                        var colspan = $('#table-{{$name}} thead tr th').length;
                                        $('#table-{{$name}} tbody').html("<tr class='trNull'><td colspan='" + colspan + "' align='center'>{{trans('locale.table_data_not_found')}}</td></tr>");
                                    }
                                }
                            }
                            ...
                            function validateForm{{$name}}() {
                                ...
                                $('#panel-form-{{$name}} .required').each(function () {
                                    var v = $(this).val();
                                    if (v == '') {
                                        swal("{{trans('locale.alert_warning')}}", "{{trans('locale.please_complete_the_form')}}", "warning");
                                        is_false += 1;
                                    }
                                })
                                @foreach($form['columns'] as $c)
                                var amount = $('#{{$name.$c["name"]}}').val();
                                @if($c['type']=='number')
                                amount = amount.replaceAll(',', '');
                              
                                 if(isNaN(amount)){
                                    is_false += 1;
                                    var col_name = '{{$c["label"]}}';
                                    swal("{{trans('locale.alert_warning')}}", "{{trans($c["label"].' must be a Number')}}", "warning");
                                 }
                                 
                                @endif
                                @endforeach
                                // if(amount) var len = amount.length;
                                // while(--len > 0){
                                ...
                                //     is_false += 1;
                                //     swal("{{trans('locale.alert_warning')}}", "{{trans('Amount must be a Number')}}", "warning");
                                // }
                                if (is_false == 0) {
                                    return true;
                                } else {
                            ...
                            function addToTable{{$name}}() {
                                ...
                                @foreach($form['columns'] as $c)
                                    ...
                                    @endif
                                @endforeach
                                    trRow += "<td width='90px' style='position: sticky; position: -webkit-sticky; right: 0px;background-color: white;'>" +
                                    "<a href='#panel-form-{{$name}}' onclick='editRow{{$name}}(this)' class='btn btn-primary'><i class='fas fa-pencil-alt'></i></a> " +
                                    "<a href='javascript:void(0)' onclick='deleteRow{{$name}}(this)' class='btn btn-danger'><i class='fa fa-trash'></i></a></td>";
                                trRow += '</tr>';
                ...
                <div class="table-responsive">
                    <table id='table-{{$name}}' class='table table-striped table-bordered'>
                        ...
                        <tr>
                            @foreach($form['columns'] as $col)
                                ...
                                if (isset($col['width']) && $col['width']) {
                                    // dd($col['width']);
                                    $minwidth = 'min-width: '.$col['width'].'px;width: '.$col['width'].'px;';
                                }
                                ?>
                                ...
                                @endif
                            @endforeach
                            <th style="min-width:120px;width:120px;position: sticky; position: -webkit-sticky; right: 0px;background-color: white;">{{trans('locale.action_label')}}</th>
                        </tr>
                            ...
                            <?php $name_column = $name.$col['name']; ?>
                                        ...
                                        <div class="modal-dialog {{ $col['datamodal_size']=='large'?'modal-lg':'' }} " role="document">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h4 class="modal-title"><i class='fa fa-search'></i> {{trans('locale.datamodal_browse_data')}} {{$col['label']}}</h4>
                                ...
                                <td> 
                                    <input id='{{$name_column}}' type='text'
                                           {{ isset($col['min'])?"min='".$col['min']."'":"" }} {{ isset($col['max'])?"max='$col[max]'":"" }} name='child-{{$col["name"]}}'
                                           class='form-control decimal text-right {{isset($col['required'])?"required":""}}'
                                            {{(isset($col['readonly']) && $col['readonly']===true)?"readonly":""}}
                                            value="{{ number_format($col['value'],$col['decimals']) }}"
                                    />
                                    ...
                                    <script type="text/javascript">
                                        @if($readonly == '')
                                            ...
                                            {
                                                var value = $(this).val();
                                                $(this).val(numberToString(value, {{(isset($col['decimals'])) ? $col['decimals']:0}}));
                                            });
                                            function setNumberFormat{{$name_column}}(value, object)
                                            {
                                                object.val(numberToString(value, {{(isset($col['decimals'])) ? $col['decimals']:0}}));
                                            }
                                        @endif
                                    </script>
                                    @endpush
                                ...
                                <td>
                                    <textarea id='{{$name_column}}' name='child-{{$col["name"]}}'
                                              class='form-control {{isset($col['required'])?"required":""}}' {{($col['readonly'] && $col['readonly']===true)?"readonly":""}} ></textarea>
                                </td>
                                ...
                                <td>
                                    ...
                                    <div id="loading-{{$name_column}}" class='text-info' style="display: none">
                                        <i class='fa fa-spin fa-spinner'></i> {{trans('locale.text_loading')}}
                                    </div>
                                        ...
                                        <script type="text/javascript">
                                            ...
                                            function prepareUpload{{$name_column}}(event) {
                                                ...
                                                if (filesize > max_size) {
                                                    sweetAlert('{{trans("locale.alert_warning")}}', '{{trans("locale.your_file_size_is_too_big")}}', 'warning');
                                                    return false;
                                                }
                                                filename = $('#fake-upload-{{$name_column}}').val().replace(/C:\\fakepath\\/i, '');
                                                var extension = filename.split('.').pop().toLowerCase();
                                                var img_extension = ['jpg', 'jpeg', 'png', 'gif', 'bmp'];
                                                var available_extension = "{{config('locale.UPLOAD_TYPES')}}".split(",");
                                                var is_image_only = {{ ($col['upload_type'] == 'image')?"true":"false" }};
                                                if (is_image_only) {
                                                    if ($.inArray(extension, img_extension) == -1) {
                                                        sweetAlert('{{trans("locale.alert_warning")}}', '{{trans("locale.your_file_extension_is_not_allowed")}}', 'warning');
                                                        return false;
                                                    }
                                                } else {
                                ...
                                @elseif($col['type']=='select2')
                            
                                <td>
                    
                                    @if(!isset($col['dataenum']) && !isset($col['datatable']) && !isset($col['api_table']))
                                        <em>{{trans('locale.there_is_no_option')}}</em>
                                    @else
                                    <div>
                                        <select style='width:100%' class='select2 form-control form-control-sm custom-select asd' id="{{$name.$col['name']}}" name="{{$name.$col['name']}}"
                                            {{$required}} {{$readonly}} {{$disabled}}>
                                        <option value=''>{{trans('locale.text_prefix_option')}} {{$col['label']}}</option>
                                        <?php
                                        ...
                                        if (isset($col['datatable'])) {
                                            ...
                                            if(isset($col['datatable_id']))
                                                $select_id = $col['datatable_id'];

                                            $result = DB::table($select_table)->select($select_id, $select_title)->distinct();
                                            
                                            $select_where = isset($col['datatable_where']) ? $col['datatable_where'] : "";
                                            if ($select_where) {
                                            ...
                                            foreach ($result as $r) {
                                                ...
                                                $option_value = $r->{$select_id};
                                                $selected = $option_value == $value ? "selected" : "";
                                                echo "<option ".$selected." value='".$option_value."'>".$option_label."</option>";
                                            }
                                        }else if(isset($col['api_table'])){
                                            $result = \MITBooster::fetchDataFromAPI($col['api_table'], (isset($col['api_where']) ? $col['api_where']:null), (isset($col['api_orderby']) ? $col['api_orderby']:null)  );
                                            if(is_array($result)) {
                                                foreach ($result as $r) {
                                                    $option_label = $r['text'];
                                                    $option_value = $r['val'];
                                ...
                                <td>
                                            ...
                                            <script type="text/javascript">
                                                $(function () {
                                                    ...
                                                    $('#{{$name.$col['parent_select']}}, input:radio[name={{$name.$col['parent_select']}}]').change(function () {
                                                        ...
                                                        var value = "{{$value}}";
                                                        if (fk_value != '') {
                                                            $current.html("<option value=''>{{trans('locale.text_loading')}} {{$col['label']}}");
                                    ...
                                    <select id='{{$name_column}}' name='child-{{$col["name"]}}'
                                            ...
                                            {{($col['readonly'] && $col['readonly']===true)?"readonly":""}} {{($col['disabled']===true)?"disabled":""}}
                                            style="width: 100%"
                                    >
                                        <?php
                               ...
                               @elseif($col['type']=='date')
                                <td>
                                    <input id='{{$name_column}}' type='text' {{(isset($col['readonly']) && $col['readonly']===true)?"readonly":""}}
                                           {{ (isset($col['max']))?"maxlength='".$col['max']."'":"" }} name='child-{{$col["name"]}}'
                                           class='form-control  not-focus input_date {{isset($col['required'])?"required":""}}'
                                    />
                                </td>
                                    @push('bottom')
                                        @if (App::getLocale() != 'en')
                                ...
                                <td>
                                    ...
                                    <input id='{{$name_column}}' type='text' {{(isset($col['readonly']) && $col['readonly']===true)?"readonly":""}} 
                                           {{ (isset($col['max']))?"maxlength='".$col['max']."'":"" }} name='child-{{$col["name"]}}'
                                           class='form-control  not-focus flatpickr {{isset($col['required'])?"required":""}}'
                                    />
                                </td>
                                    @push('head')
                               ...
                               @else
                                ...
                                @endif
                        @endforeach
                            <td style='position: sticky; position: -webkit-sticky; right: 0px;background-color: white;'>
                        ...
                        <tr class='{{$name}}-row'>
                            @foreach($form['columns'] as $col)
                                    ...
                                    if ($col['type'] == 'select') {
                                        ...
                                        echo "<span class='td-label'>";
                                        if ($col['type'] == 'number') {
                                            echo number_format($d->{$col['name']}, (isset($col['decimal']))?$col['decimal']:0);
                                        } else {
                                            echo $d->{$col['name']};
                                        }
                                        echo "</span>";
                                    ...
                                    ?>
                            @endforeach
                            <td width="100px" style='position: sticky; position: -webkit-sticky; right: 0px;'>
                                <a href='#panel-form-{{$name}}' onclick='editRow{{$name}}(this)' class='btn btn-primary'><i
                                            class='fas fa-pencil-alt'></i></a>
                                <a href='javascript:void(0)' onclick='deleteRow{{$name}}(this)' class='btn btn-danger'><i
                        ...
                        @if(count($data_child)==0)
                            <tr class="trNull">
                                <td colspan="{{count($form['columns'])+1}}" align="center">{{trans('locale.table_data_not_found')}}</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
